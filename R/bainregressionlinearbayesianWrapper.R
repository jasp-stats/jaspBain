...
BainRegressionLinearBayesian <- function(
   ...
   optionsWithFormula <- c("covariates", "dependent", "model")
   for (name in optionsWithFormula) {
      if ((name %in% optionsWithFormula) && inherits(options[[name]], "formula")) options[[name]] = jaspBase::jaspFormula(options[[name]], data)   }

   return(jaspBase::runWrappedAnalysis("jaspBain", "BainRegressionLinearBayesian", "BainRegressionLinearBayesian.qml", options, version, TRUE))
}